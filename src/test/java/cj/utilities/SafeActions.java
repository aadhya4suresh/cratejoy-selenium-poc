...
public class SafeActions extends Sync {
	...
	public void jsClickOnElement(String cssSelector) {
		try {
			...
			log.error("some exception occured while trying to click on locator with css:" + cssSelector
					+ " using Jsclick" + "\n" + e.getMessage());
			Assert.fail("some exception occured while trying to click on locator with css:" + cssSelector
					+ " using Jsclick" + "\n" + e.getMessage());
		}
	}
	...
	public List<WebElement> LocatorWebElements(By Locator) {
		...
		try {
			...
			log.error("Some exception occured while retriving web elements of a locator, exception message: "
					+ e.getMessage());
			Assert.fail("Some exception occured while retriving web elements of a locator, exception message: "
					+ e.getMessage());
		}
		return list;
	}
	...
	public void hitEnter(By locator, int... optionWaitTime) {
		int waitTime = getWaitTime(optionWaitTime);
		scrollIntoElementView(locator);
		WebElement element = driver.findElement(locator);
		setHighlight(element);
	...
	public void safeClear(By locator, int... optionWaitTime) {
		try {
			int waitTime = getWaitTime(optionWaitTime);
			if (isElementPresent(locator, waitTime)) {
				scrollIntoElementView(locator);
			...
			System.out.println("Exception message: " + e.getMessage());
			log.error("Unable to clear the text in field with locator -" + locator
					+ UtilityMethods.getStackTrace());
			Assert.fail("Unable to clear the text in field with locator -" + locator
					+ UtilityMethods.getStackTrace());
		}
	}
	
	
	public void closeTheWindow(int num) {
		try {
			//int numWindow = driver.getWindowHandles().size();
			//String[] window = (String[]) driver.getWindowHandles().toArray(new String[numWindow]);
			//driver.switchTo().window(window[num]);
	...
	public void safeClick1(By locator, int... optionWaitTime) {
	    try {
	        int waitTime = getWaitTime(optionWaitTime);
	        if (waitUntilClickable1(locator, waitTime)) {
	            WebElement element = driver.findElement(locator);
	            element.click();
	            log.info("Clicked on the element " + locator);
	        } else {
	            log.error("Unable to click the element " + locator);
	            Assert.fail("Unable to click the element " + locator);
	        }
	    } catch (WebDriverException e) {
	        log.error("Exception occurred while clicking the element " + locator + ": " + e.getMessage());
	        Assert.fail("Exception occurred while clicking the element " + locator + ": " + e.getMessage());
	    }
		catch (Exception e) {
		System.out.println("Exception message: " + e.getMessage());
		log.error("Element " + locator + " was not clickable" + UtilityMethods.getStackTrace());
