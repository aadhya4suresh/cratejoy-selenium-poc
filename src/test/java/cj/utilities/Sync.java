...
import cj.utilities.TimeOuts;
import cj.utilities.UtilityMethods;

public class Sync extends TestngListener implements TimeOuts

{
	 ...
	 *
	 *
	 * @param driver           (WebDriver) The driver object to be used to wait and
	 *                         find the element
	 * @param javaScript       (String) The javaScript condition we are waiting.
	 *                         e.g. "return (xmlhttp.readyState >= 2 &&
	 *                         xmlhttp.status == 200)"
	 * @param timeOutInSeconds (int) The time in seconds to wait until returning a
	 *                         failure
	 * @return True (boolean) if javaScript condition is satisfied within
	 *         timeOutInSeconds
	 **/
	 ...
	 * 
	 * @param driver    (WebDriver) The driver object to be used to wait and find
	 *                  the element
	...
	public boolean isElementPresent(By locator, int waitTime) {
		...
		try {
			...
			System.out.println("NoSuchElement Exception message:" + e.getMessage());
			log.info("Element " + locator + " was not found in DOM" + UtilityMethods.getStackTrace());
		} catch (TimeoutException e) {
			System.out.println("Timeout Exception message:" + e.getMessage());
			log.info(
					"Element " + locator + " was not displayed in time - " + waitTime + UtilityMethods.getStackTrace());
		} catch (Exception e) {
			System.out.println("Exception message:" + e.getMessage());
			log.error("Element " + locator + " is not displayed" + UtilityMethods.getStackTrace());
			// Assert.fail("Element " + locator + " is not
			// displayed"+UtilityMethods.getStackTrace());
		}
		setImplicitWait(IMPLICITWAIT);
		return bFlag;
	}
	...
	public boolean waitUntilClickable(By locator, int... optionWaitTime) {
		...
		//	WebDriverWait wait = new WebDriverWait(driver, waitTime);
			...
			log.error(
					"Element " + locator + " was not clickable in time - " + waitTime + UtilityMethods.getStackTrace());
			Assert.fail(
					"Element " + locator + " was not clickable in time - " + waitTime + UtilityMethods.getStackTrace());
		} catch (Exception e) {
	...
	public boolean waitUntilVisible(By locator, int... optionWaitTime) {
		int waitTime = getWaitTime(optionWaitTime);
		boolean bFlag = false;
		nullifyImplicitWait();
		try {
			...
			if (driver.findElement((locator)).isDisplayed()) {
				bFlag = true;
				log.info("Element " + locator + " is displayed and is visible");
			}
		}
			...
			log.error("Element " + locator + " was not visible in time - " + waitTime + UtilityMethods.getStackTrace());
			Assert.fail(
					"Element " + locator + " was not visible in time - " + waitTime + UtilityMethods.getStackTrace());
		} catch (Exception e) {
			log.error("Element " + locator + " was not visible" + UtilityMethods.getStackTrace());
			Assert.fail("Element " + locator + " was not visible" + UtilityMethods.getStackTrace());
		}
		setImplicitWait(IMPLICITWAIT);
		return bFlag;
	}
	 ...
	 * @param optionWaitTime (int) The time in seconds to wait until returning a
	 *                       failure
	...
	/**
	 * Purpose- Wait for an element till it is either invisible or not present on
	 * the DOM.
	 * 
	 * @param driver    (WebDriver) The driver object to be used to wait and find
	 *                  the element
	 ...
	 *
	 * @param driver           - The driver object to be used to wait and find the
	 *                         element
	 * @param timeOutInSeconds - The time in seconds to wait until returning a
	 *                         failure
	 ...
	 * 
	 * @param driver (WebDriver) The driver object to be used to wait and find the
	 *               element
	...
	public boolean isElementInvisible(By locator, int... optionWaitTime) {
		int waitTime = getWaitTime(optionWaitTime);
		boolean isNotVisible = false;
		log.info("Waiting until element " + locator + " is invisible");
		try {
	...
	public void waitUntilRespondCodeMatches() {
		...
		devTool.addListener(Network.responseReceived(), responseReceieved -> {
			...
			int resCode = responseReceieved.getResponse().getStatus();
			System.out.println(resCode);
		});
		expRespCode = 200;

		if (resCode == expRespCode) {

			System.out.println("Response code" + resCode + "match with" + expRespCode);
		} else {

			System.out.println("Response code" + resCode + "mismatch with" + expRespCode);
		}
	}

	public boolean isElementDisabled(By locator) {
		log.info("Verifying if element " + locator + " is disable");
		boolean isDisabled = false;
		setImplicitWait(IMPLICITWAIT);
		try {
			if (isElementPresent(locator)) {
				isDisabled = driver.findElement(locator).isEnabled();
			}
		} catch (Exception e) {
			log.error("Exception occured while determining state of " + locator + UtilityMethods.getStackTrace());
		}
	 ...
	 * =================================================================
	 */
	public boolean isElementVisible12(By locator) {
		// int waitTime = getWaitTime(optionWaitTime);
		boolean bFlag = false;
		...
		try {
			if (driver.findElement((locator)).isDisplayed()) {
				System.out.println(locator + "visible ");
				bFlag = true;
				// log.info("Element " + locator + " is displayed");
			}
		} catch (Exception e) {

			Assert.fail("Locator" + locator + "is not dispalyed");
	...
	public boolean waitUntilClickable1(By locator, int... optionWaitTime) throws TimeoutException {
	    int waitTime = getWaitTime(optionWaitTime);
	    boolean bFlag = false;
	    nullifyImplicitWait();
	    try {
	        ...
	        log.error("Exception occurred while waiting for the element " + locator + ": " + e.getMessage());
	        Assert.fail("Exception occurred while waiting for the element " + locator + ": " + e.getMessage());
	    }
	 catch (Exception e) {
		log.error("Element " + locator + " was not clickable" + UtilityMethods.getStackTrace());
		Assert.fail("Element " + locator + " was not clickable" + UtilityMethods.getStackTrace());
